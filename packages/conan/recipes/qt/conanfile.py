...
Examples = bin/datadir/examples""" % self.conanfile.deps_cpp_info[
            "qt"
        ].rootpath.replace(
            "\\", "/"
        )
...
class QtConan(ConanFile):
    ...
    options = {
        ...
        "with_libjpeg": ["libjpeg", "libjpeg-turbo", "qt", False],
        "with_libpng": ["qt", "libpng", False],
    ...
    default_options.update({module: True for module in _submodules})
    default_options["qtnetworkauth"] = False
    default_options["qtpurchasing"] = False
    default_options["qtremoteobjects"] = False
    ...
    def config_options(self):
        ...
        if (
            ...
            and tools.Version(self.settings.compiler.version) < "5.3"
        ):
    ...
    def configure(self):
        # if self.settings.os != "Linux":
        #    args += ["self.options.with_libiconv = False # QTBUG-84708
    ...
    def build(self):
        ...
        args.append("-v")
        args.append(
            "-archdatadir  %s" % os.path.join(self.package_folder, "bin", "archdatadir")
        )
        args.append(
            "-datadir  %s" % os.path.join(self.package_folder, "bin", "datadir")
        )
        args.append(
            "-sysconfdir  %s" % os.path.join(self.package_folder, "bin", "sysconfdir")
        )
        ...
        if not self.options.shared:
            ...
            if self.settings.compiler == "Visual Studio":
                if (
                    ...
                    or self.settings.compiler.runtime == "MTd"
                ):
        ...
        if not self.options.openssl:
            ...
            args += ["-openssl"]
            args += ["-I /usr/include/openssl11", "-L /usr/lib64/openssl11"]
        ...
        args.append(
            ...
            % " ".join(
                "%s%s" % (lib_arg, l)
                for package in self.deps_cpp_info.deps
                for l in self.deps_cpp_info[package].lib_paths
            )
        )
        ...
        if "libmysqlclient" in self.deps_cpp_info.deps:
            args.append(
                '-mysql_config "%s"'
                % os.path.join(
                    self.deps_cpp_info["libmysqlclient"].rootpath, "bin", "mysql_config"
                )
            )
        if "libpq" in self.deps_cpp_info.deps:
            args.append(
                '-psql_config "%s"'
                % os.path.join(self.deps_cpp_info["libpq"].rootpath, "bin", "pg_config")
            )
        ...
        with tools.chdir("build_folder"):
            with tools.vcvars(
                ...
                if self.settings.os == "Windows":
                    build_env["PATH"] = [
                        os.path.join(self.source_folder, "qt5", "gnuwin32", "bin")
                    ]
                ...
                with tools.environment_append(build_env):
                    ...
                    self.run(
                        "%s/qt5/configure %s" % (self.source_folder, " ".join(args)),
    ...
    def _cmake_executables_file(self):
        return os.path.join(
            "lib", "cmake", "Qt5Core", "conan_qt_executables_variables.cmake"
        )
...
Examples = bin/datadir/examples"""
    ...
    def package_id(self):
        ...
        if (
            self.options.multiconfiguration
    ...
    def package_info(self):
        ...
        def _create_module(module, requires=None):
            if requires is None:
                requires = []
            componentname = "qt%s" % module
            assert componentname not in self.cpp_info.components, (
                "Module %s already present in self.cpp_info.components" % module
            )
            ...
            self.cpp_info.components[componentname].includedirs = [
                "include",
                os.path.join("include", "Qt%s" % module),
            ]
        ...
        def _create_plugin(pluginname, libname, type, requires):
            ...
            assert componentname not in self.cpp_info.components, (
                "Plugin %s already present in self.cpp_info.components" % pluginname
            )
            self.cpp_info.components[componentname].names[
                "cmake_find_package"
            ] = pluginname
            ...
            self.cpp_info.components[componentname].libdirs = [
                os.path.join("bin", "archdatadir", "plugins", type)
            ]
            self.cpp_info.components[componentname].includedirs = []
        ...
        if self.options.get_safe("with_mysql", False):
            _create_plugin("QMySQLDriverPlugin", "qsqlmysql", "sqldrivers", [])
        ...
        if self.options.qtdeclarative:
            ...
            self.cpp_info.components[
                "qtQmlImportScanner"
            ].requires = _get_corrected_reqs(["Qml"])
            if self.options.gui:
        ...
        if self.options.qtwayland and self.options.gui:
            _create_module("WaylandClient", ["Gui"])
            _create_module("WaylandCompositor", ["Gui"])
        ...
        qt_version = tools.Version(self.version)
        ...
        if self.options.qtmultimedia:
            ...
            if self.settings.os == "Linux":
                _create_module(
                    "MultimediaGstTools", ["Multimedia", "MultimediaWidgets", "Gui"]
                )
                _create_plugin(
        ...
        self.cpp_info.components["qtCore"].builddirs.append(
            os.path.join("bin", "archdatadir", "bin")
        )
        self.cpp_info.components["qtCore"].build_modules["cmake_find_package"].append(
            self._cmake_executables_file
        )
        self.cpp_info.components["qtCore"].build_modules[
            "cmake_find_package_multi"
        ].append(self._cmake_executables_file)
    ...
    def _gather_libs(self, p):
        ...
        libs = [
            "-l" + i
            for i in self.deps_cpp_info[p].libs + self.deps_cpp_info[p].system_libs
        ]
